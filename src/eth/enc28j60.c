...
uint8_t enc28j60ReadOp(uint8_t op, uint8_t address)
{
    ...
    enableChip();
    spi_io(op | (address & ADDR_MASK));
    result = spi_io(0x00);
    if (address & 0x80) result = spi_io(0x00);
    disableChip();
    return result;
}
...
void enc28j60Init(uint8_t* macaddr)
{
    ...
    // 16-bit transfers, must write low uint8_t first
    ...
    // Type     ETH.DST
    // ARP      BROADCAST
    // 06 08 -- ff ff ff ff ff ff -> ip checksum for theses uint8_ts=f7f9
    ...
    // enable automatic padding to 60uint8_ts and CRC operations
    ...
    // do bank 3 stuff
    // write MAC address
    // NOTE: MAC address in ENC28J60 is uint8_t-backward
...
void enc28j60PacketSend(uint16_t len, uint8_t* packet)
{
    ...
    // write per-packet control uint8_t (0x00 means use macon3 settings)
...
// Returns: Packet length in uint8_ts if a packet was retrieved, zero otherwise.
